...
describe('executeAccessScopes', () => {
	...
	it('should throw an error for unsupported operation', async () => {
		...
		it("should execute 'queryAccessScopesExternal' operation with default parameters successfully", async () => {
			const mockContext: any = {
		...
		it("should execute 'queryAccessScopesExternal' operation with non-empty parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
					if (paramName === 'operation') return 'queryAccessScopesExternal';
		...
		it("should execute 'listAccessScopesExternal' operation with default parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
		...
		it("should execute 'listAccessScopesExternal' operation with non-empty parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
					if (paramName === 'operation') return 'listAccessScopesExternal';
					if (['ids', 'id', 'idsString', 'user_ids', 'cids', 'uuids', 'device_ids', 'composite_ids', 'event_ids', 'tags'].includes(paramName)) return 'id1, id2';
