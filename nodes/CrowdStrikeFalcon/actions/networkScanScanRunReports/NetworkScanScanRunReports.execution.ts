...
export async function executeNetworkScanScanRunReports(
	...
	if (operation === 'getScanRunReports') {
		return await handleGetScanRunReports(this, index, falconClient);
	}
	throw new NodeOperationError(this.getNode(), `Operation ${operation} is not supported.`); as any)), `Operation ${operation} is not supported for Network Scan Scan Run Reports.`);
	}
}
