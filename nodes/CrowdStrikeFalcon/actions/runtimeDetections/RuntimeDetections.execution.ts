...
export async function executeRuntimeDetections(
	...
	if (operation === 'getRuntimeDetectionsCombinedV2') {
		return await handleGetRuntimeDetectionsCombinedV2(this, index, falconClient);
	}
	throw new NodeOperationError(this.getNode(), `Operation ${operation} is not supported.`); as any)), `Operation ${operation} is not supported for Runtime Detections.`);
	}
}
