...
/**
 * Comprehensive unit test suite for executeSensorUsageApi operations.
 */
describe('executeSensorUsageApi', () => {
	...
	it('should throw an error for unsupported operation', async () => {
		...
		it("should execute 'getSensorUsageHourly' operation with default parameters successfully", async () => {
			const mockContext: any = {
		...
		it("should execute 'getSensorUsageHourly' operation with non-empty parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
					if (paramName === 'operation') return 'getSensorUsageHourly';
		...
		it("should execute 'getSensorUsageWeekly' operation with default parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
		...
		it("should execute 'getSensorUsageWeekly' operation with non-empty parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
					if (paramName === 'operation') return 'getSensorUsageWeekly';
					if (['ids', 'id', 'idsString', 'user_ids', 'cids', 'uuids', 'device_ids', 'composite_ids', 'event_ids', 'tags'].includes(paramName)) return 'id1, id2';
