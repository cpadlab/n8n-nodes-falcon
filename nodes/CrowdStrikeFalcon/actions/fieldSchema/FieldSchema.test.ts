...
/**
 * Comprehensive unit test suite for executeFieldSchema operations.
 */
describe('executeFieldSchema', () => {
	...
	it('should throw an error for unsupported operation', async () => {
		...
		it("should execute 'fdrschemaEntitiesFieldGet' operation with default parameters successfully", async () => {
			const mockContext: any = {
		...
		it("should execute 'fdrschemaEntitiesFieldGet' operation with non-empty parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
					if (paramName === 'operation') return 'fdrschemaEntitiesFieldGet';
		...
		it("should execute 'fdrschemaQueriesFieldGet' operation with default parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
		...
		it("should execute 'fdrschemaQueriesFieldGet' operation with non-empty parameters successfully", async () => {
			const mockContext: any = {
				getNodeParameter: jest.fn((paramName: string, index: number, fallback?: any) => {
					if (paramName === 'operation') return 'fdrschemaQueriesFieldGet';
					if (['ids', 'id', 'idsString', 'user_ids', 'cids', 'uuids', 'device_ids', 'composite_ids', 'event_ids', 'tags'].includes(paramName)) return 'id1, id2';
